...
L1Controller::L1Controller()
{
    ...
    odom_sub = n_.subscribe("/amcl_pose", 1, &L1Controller::odomCB, this);
...
void L1Controller::odomCB(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr &odomMsg)
{
    if (AllowReverse)
    {
        odom = *odomMsg; // const geometry_msgs::PoseWithCovarianceStamped::ConstPtr
...
{
    if (sub_locate_data != 11 && sub_locate_data != 12)
    {
        map_path = *pathMsg;
    }
}
...
void L1Controller::goalCB(const geometry_msgs::PoseStamped::ConstPtr &goalMsg)
{
    ...
    try
    {
        ...
        tf_listener.transformPose("odom", ros::Time(0), *goalMsg, "map", goal);
        goal_received = true;
        goal_reached = false;
        switch (goalLocation)
        {
        case Start:
            ...
            break;
        case Load:
            ROS_INFO("---------------------------x of Load:%f---------------------------[before]", goal.pose.position.x);
            goal.pose.position.y -= (1 * Lfw);
            ROS_INFO("---------------------------x of Load:%f---------------------------[after]", goal.pose.position.x);
        ...
        nav_goal_pos_forward.x = goal.pose.position.x;
        nav_goal_pos_forward.y = goal.pose.position.y;
        nav_goal_pos_forward.z = 0.0;

        nav_goal_pos_back.x = goal.pose.position.x;
        nav_goal_pos_back.y = goal.pose.position.y;
