...
#include "dynamicParamService/L1_dynamicConfig.h"
...
void L1Controller::goalReachingCB(const ros::TimerEvent&)
{
    ...
    if(goal_received)
    {
        ...
        if(car2goal_dist < 0.18)
        {
            ROS_INFO("goal receive become flase here------------");
            goal_reached = true;
            goal_received = false;
            ROS_INFO("Goal Reached !");
        }
...
void L1Controller::controlLoopCB(const ros::TimerEvent&)
{
    ...
    if(goal_received)
    {
        ...
        if(foundForwardPt)
        {
            // ROS_INFO("Base angle------------%.4f",eta);
            twist_cmd.angular.z = getSteeringAngle(eta)*angle_gain;
            /*Estimate Gas Input*/
            if(!goal_reached)
            {
                // ROS_INFO("Base speed------------");
                twist_cmd.linear.x = base_speed;
            }
        }
    }
	if (goal_received || goal_reached)
	{
		pub_.publish(twist_cmd);
