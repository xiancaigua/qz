...
private:
    ...
    path_sub, goal_sub, hypath_sub, locate_sub;
...
void L1Controller::goalCB(const geometry_msgs::PoseStamped::ConstPtr &goalMsg)
{
    ...
    try
    {
        ...
        // nav_goal_pos_forward.x = odom_goal.pose.position.x;
        // nav_goal_pos_forward.y = odom_goal.pose.position.y;
        // nav_goal_pos_forward.z = 0.0;

        // nav_goal_pos_back.x = odom_goal.pose.position.x;
        // nav_goal_pos_back.y = odom_goal.pose.position.y;
...
void L1Controller::controlLoopCB(const ros::TimerEvent &)
{
    ...
    if (goal_received || goal_reached)
    {
        ...
        {
            ackermann_cmd.steering_angle += PI/36;//第二段手动加5度的偏移
        }
