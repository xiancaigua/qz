...
u1 = 0.0
#1.0 -0.62
goal_x = np.array([-0.4, -2.2, -2.2, 0])
goal_y = np.array([-0.18, -0.8, -0.3, 0])
# 0.55 0.85
# goal2_x = -2.0
# goal2_y = -0.46
"""
...
class Config(object):
    ...
    def __init__(self,path):
        if path == 1:
            ...
            self.max_yawrate = 120 * 3.1415926 / 180.0 # [rad/s] # 最大角速度120 115
            self.max_accel = 0.2 # [m/ss] # 最大加速度0.15
            self.max_dyawrate = 80 * 3.1415926 / 180.0 # [rad/ss] # 最大角加速度45  原来是3
            self.v_reso = 0.01 # [m/s] 速度分辨率0.01
            self.yawrate_reso = 0.2 * 3.1415926 / 180.0 # [rad/s] 角度分辨率 原来的是0.2
            self.dt = 0.05 # [s] # 采样时间
            self.predict_time = 1.0  # [s] # 模拟时间 1.0
...
def calc_angle_cost(trajectory,config):
    cost = abs( - trajectory[2])
    return config.angle_cost_gain * cost

def calc_dy_cost(trajectory,goal,config):
    cost  = abs(trajectory[1] - goal[1])
...
def calc_trajectory(x_init, v, w, config , path):
    x = np.array(x_init)
    trajectory = np.array(x)
    time = 0
    while time <= config.predict_time:
        x = motion(x, [v, w], config.dt ,path)
...
def calc_final_input(x, u, vr, config, goal,path):
    ...
    for v in np.arange(vr[0], vr[1], config.v_reso):
        for w in np.arange(vr[2], vr[3], config.yawrate_reso):
            ...
            trajectory = calc_trajectory(x_init, v, w, config , path)
            # calc cost
...
def main():
    ...
    global flag_debug
    goal2 = np.array([goal_x[flag_debug] , goal_y[flag_debug]])
    msg = PoseStamped()
    msg.header.seq = 0
    msg.header.stamp.secs = 0
    msg.header.frame_id = 'map'
