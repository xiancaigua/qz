...
class YoLov5TRT(object):
    ...
    def infer(self, raw_image_generator):
        ...
        return batch_image_raw, end - start,box
...
class inferThread(threading.Thread):
    ...
    def run(self):
        batch_image_raw, use_time,box = self.yolov5_wrapper.infer(self.yolov5_wrapper.get_raw_image(self.image))
        print(box)
...
if __name__ == "__main__":
    ...
    image = []
    a = cv2.imread("images/green1000.jpg")
    image.append(a)

    PLUGIN_LIBRARY = "build/libmyplugins.so"
    engine_file_path = "build/traffic.engine"
    ...
    try:
        ...
        while cam.isOpened():
            ret,frame = cam.read()
            if ret:
                thread = inferThread(yolov5_wrapper,frame)
                thread.start()
                thread.join()
